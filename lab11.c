...
#define MAX_FILE_SIZE 512
  ...
  size_t message_len = read_all_bytes(sign_path, signature, MAX_FILE_SIZE);
  ...
  // step 4a: initialize the digest verify operation
  if (EVP_DigestVerifyInit(mdctx, NULL, EVP_sha256(), NULL, pubkey) != 1) {
    fprintf(stderr, "Error initiliazing digest verify\n");
    ERR_print_errors_fp(stderr);
  ...
  if (verify_result == 1) {
    ...
    return 0; // Signature verification failed (tampered or wrong signature)
  } else {
