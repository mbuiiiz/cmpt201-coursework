...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  // argumenet safety checks
  if (input == NULL || output == NULL || result_count == NULL)
    return;

  // if there is nothing inside output array yet, add the first element found
  // inside the array
  if (*result_count == 0) {
  ...
  // find if any doubled value is in the array
  ...
  // if nothing is found just add it into the end of output array
  *result_count++;
  output[*result_count].doubled_value = input->doubled_value;
  output[*result_count].line_numbers[output[*result_count].count] =
      input->line_number;
