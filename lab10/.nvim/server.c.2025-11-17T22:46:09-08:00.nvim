...
The client is sending data through TCP over IPv4 because there is AF_INET
mentioned. It connects through port 8001 on local host

2. What data is the client sending to the server?
Each client is sending 5 strings of 1024 bytes char
4 clients x 5 messages each = 20 messages is sent to the server

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
The run_accceptor thread received a struct accept_args as an argument
run: atomic boolean flag to tell thread acceptor to accept new connections
list_handle: pointer to shared message list where all msgs are stored
list_lock: pointer to the mutex that protects the shared list from race cond

2. How are received messages stored?
It is stored inside a singly-linked list with tracking the last element

3. What does `main()` do with the received messages?
-wait until enough msgs are received to stop the acceptor (aargs = false)
-join the acceptor thread (wait for it to finish)
-free stuffs and destroy mutex after use
-return and verify if all messages are collected

4. How are threads used in this sample code?
-One acceptor thread (run_acceptor) that accepts incoming connections
-Multiple worker threads to handle I/O
-Main thread for coordination and message processing

How are sockets made non-blocking ?
How are sockets are made non-blocking?
Why are these sockets made non-blocking? What purpose does it serve?

server socket and client socket are non-blocking so accept() return immediately,
so we can continuely check if aargs->run is true
...
int main() {
  ...
  printf("Waiting for 20 messages\n");

  uint32_t expected_messages = MAX_CLIENTS * NUM_MSG_PER_CLIENT;
  ...
  while (1) {
    ...
    if (current_count >= expected_messages) {
      break;
    }
  ...
  if (list_handle.count != MAX_CLIENTS * NUM_MSG_PER_CLIENT) {
    ...
    printf("Enough messages were received\n");
  }
