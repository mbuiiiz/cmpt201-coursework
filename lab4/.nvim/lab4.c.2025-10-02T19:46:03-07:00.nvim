...
int main() {
  ...
  my_stack1->size = BLOCK_SIZE;
  my_stack1->next = NULL;

  // fill up the memory from stack1
  char *data_ptr1 = (char *)(my_stack1 + 1);
  memset((char *)data_ptr1, 0, 112);

  // initialize second block
  struct header *my_stack2 = (struct header *)(my_stack1 + 128);
  my_stack2->size = BLOCK_SIZE;
  my_stack2->next = my_stack1;

  // fill up the memory from stack2
  ...
  print_out("first block next: %p\n", &my_stack1->next,
            sizeof(my_stack1->next));

  print_out("second block size: %llu\n", &my_stack2->size,
            sizeof(my_stack2->size));
  print_out("second block next: %p\n", &my_stack2->next,
            sizeof(my_stack2->next));
